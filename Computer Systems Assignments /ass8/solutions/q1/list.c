...
/**
 * Map function f onto in_list placing results in out_list.
...
void list_map1 (void (*f) (element_t*, element_t), struct list* out_list, struct list* in_list) {
  ...
  for (int i = 0; i < in_list->len; i++) {
    element_t rv = 0;
    f (&rv, in_list->data [i]);
    list_append (out_list, rv);
  }
}

/**
 * Map function f onto list0 and in_list1 placing results in out_list.
 * The target list out_list must not be one of in_list1 or in_list2.  
...
void list_map2 (void (*f) (element_t*, element_t, element_t), struct list* out_list, struct list* in_list0, struct list* in_list1) {
  out_list->len = 0;
  for (int i = 0; i < ((in_list0->len < in_list1->len)? in_list0->len : in_list1->len); i++) {
    element_t rv = 0;
    f (&rv, in_list0->data [i], in_list1->data [i]);
    list_append (out_list, rv);
  }
}
