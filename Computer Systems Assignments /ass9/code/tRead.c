...
void* read (void* blockno_v) {
    // TODO
    char buf[8];
    intptr_t blkNo = (intptr_t)blockno_v;
    disk_schedule_read(buf, sizeof(buf), blkNo);
    uthread_block();
    handle_read(buf, blkNo);
...
void run (int num_blocks) {
    // TODO
    uthread_t th[num_blocks];
    for (int blockno = 0; blockno < num_blocks; blockno++)
        th[blockno] = uthread_create(read, (void*) blockno);
    for (int blockno = 0; blockno < num_blocks; blockno++)
        uthread_join(th[blockno], NULL);
}
