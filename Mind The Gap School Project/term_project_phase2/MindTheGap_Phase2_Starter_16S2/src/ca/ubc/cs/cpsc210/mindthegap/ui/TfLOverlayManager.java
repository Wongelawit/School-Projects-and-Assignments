...
public class TfLOverlayManager implements MapEventsReceiver {
    ...
    public void updateMarkerOfNearest(Station nearest) {
        ...
        Drawable closestStnIconDrawable = mapFragment.getResources().getDrawable(R.drawable.closest_stn_icon);
         List <Marker> marker= stnClusterer.getItems();
            for (Marker mrk: marker){
                if (mrk.getRelatedObject().equals(nearest)){
                    mrk.setIcon(closestStnIconDrawable);
                }else{
                    mrk.setIcon(stnIconDrawable);
                }
                if(nearest==null){
                    mrk.setIcon(stnIconDrawable);
                }
    ...
    private void markStations() {
        Drawable stnIconDrawable = mapFragment.getResources().getDrawable(R.drawable.stn_icon);
        StationManager stnManager = StationManager.getInstance();// this called creating an object of station manager
        for (Station stn: stnManager){
            StationMarker stnMarker = new StationMarker(mapView,this);
            stnMarker.setTitle(stn.getName());
            LatLon llp = stn.getLocn();
            GeoPoint gp = new GeoPoint(llp.getLatitude(), llp.getLongitude());
            stnMarker.setPosition(gp);
    ...
    private void plotLines() {
        ...
        for(Line line: longPressedStn.getLines()){
            Set<Branch> br= line.getBranches();
            for(Branch branch: br) {
                List<LatLon> ll = branch.getPoints();
                List<GeoPoint> giop = new ArrayList<>();
                for (LatLon llp : ll) {
                    GeoPoint gp = new GeoPoint(llp.getLatitude(), llp.getLongitude());
                    giop.add(gp);
                }
                Polyline polyline = new Polyline(mapView.getContext());
                polyline.setPoints(giop);
